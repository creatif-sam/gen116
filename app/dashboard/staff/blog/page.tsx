...
export default function StaffBlogPage() {
  ...
  const loadBlogs = async () => {
    ...
    if (data) {
      // Filter to show only blogs by the current staff member
      const userBlogs = data.filter((blog: Blog) => blog.author === user?.name || blog.author === user?.email);
      setBlogs(userBlogs);
    }
    setLoading(false);
  };
                  ...
                  <div key={blog.id} className="p-6 bg-gradient-to-br from-purple-900/20 to-blue-900/20 rounded-2xl border border-purple-500/20">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2 ml-6">
                        ...
                        <Link
                          href={`/dashboard/staff/blog/${blog.id}`}
